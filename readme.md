...
Spring Boot 기반 MSA 실습 프로젝트입니다.
Eureka 서비스 디스커버리, API Gateway, OAuth2 인증 서버, Kafka 비동기 이벤트로 구성되어 있으며, Vue 3 프론트엔드가 게이트웨이를 통해 각 서비스를 호출합니다.
...
### Role enum 변경 시 로그인 실패

```
No enum constant com.lecture.auth.model.User.Role.USER
```

`auth-server`는 소스가 없는 배포 이미지로, 내부 enum이 `STUDENT` / `INSTRUCTOR`만 지원합니다. DB에 다른 role 값이 저장되면 변환에 실패하며, `docker-compose.build.yml`로 실행해도 auth-server는 빌드 대상이 아니라 해결되지 않습니다.

→ 내부 role 값은 유지하고 화면 표시명만 치환합니다. `courses.category` 값은 auth-server와 무관하므로 자유롭게 변경할 수 있습니다.
...
본 실습 코드는 교육 목적으로만 제공됩니다. 상용 서비스로 사용하려면 배포 목적에 따른 보완이 필요합니다.

원본 실습 템플릿 — Sungryel Lim Ph.D. (audit@korea.ac.kr)
